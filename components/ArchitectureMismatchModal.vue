        ...
        <div v-if="mismatchData" class="space-y-4">
          ...
          <div class="grid grid-cols-2 gap-3">
            <div class="bg-slate-800/50 rounded-lg p-3 border border-slate-700">
              <div class="text-xs text-slate-400 uppercase tracking-wide mb-1">Current</div>
              <div class="font-mono text-sm text-slate-200">{{ formatArch(mismatchData.current) }}</div>
            </div>
            <div class="bg-teal-900/30 rounded-lg p-3 border border-teal-700">
              <div class="text-xs text-teal-400 uppercase tracking-wide mb-1">Imported</div>
              <div class="font-mono text-sm text-teal-200">{{ formatArch(mismatchData.imported) }}</div>
            </div>
          </div>

          <!-- Warning callout -->
          <div class="bg-amber-900/20 border border-amber-700/50 rounded-lg p-3 text-sm">
          ...
          <!-- Import details -->
          <div class="bg-slate-800/30 rounded-lg p-3 text-sm">
            <div class="flex justify-between text-slate-400">
              <span>Generation:</span>
              <span class="text-teal-300 font-mono">{{ mismatchData.pendingGeneration }}</span>
            </div>
            <div class="flex justify-between text-slate-400 mt-1">
              <span>Fitness:</span>
              <span class="text-teal-300 font-mono">{{ mismatchData.pendingGenome.fitness.toFixed(0) }}</span>
            </div>
          </div>
        </div>
